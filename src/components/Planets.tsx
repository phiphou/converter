...
function Planets({dictionary}: UnitSelectProps) {
  ...
  useEffect(() => {
    async function calculateResult() {
      ...
      // Object.entries(dictionary["diameter"] as unknown as Record<string, number>).forEach(([key, v]) => {
      //   if (!results[key]) {
      //     results[key] = {}
      //   }
      //   results[key].diameter = v / ratio
      // })

      // Object.entries(dictionary["distance"] as unknown as Record<string, number>).forEach(([key, v]) => {
      //   if (!results[key]) {
      //     results[key] = {}
      //   }
      //   results[key].distance = v / ratio
      // })
  ...
  return (
    <div className="m-10">
      <div className="flex items-center text-black dark:text-white">
        <div className="min-w-10">Si {mode === "distance" ? "la" : "le"}</div>
        <select
        ...
        <div className="ml-4 min-w-12">{planet == "sun" ? "du" : planet == "earth" ? "de la" : "de"}</div>
        <select
          value={planet}
      ...
      <div className="mt-6 flex items-center text-black dark:text-white">
        <div className="min-w-10">est {mode === "distance" ? " égale à " : "égal à "}</div>
        <input
          type="text"
          min={0}
          className="mr-4 ml-4 block w-25 rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus-within:ring-blue-500 focus:border-blue-500 focus:ring-blue-500 md:w-40 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus-within:ring-blue-500 dark:focus:border-blue-500 dark:focus:ring-blue-500"
          value={rawValue}
          onKeyDown={(e) => {
      ...
      <div className="relative mt-10 overflow-x-auto rounded-lg shadow">
